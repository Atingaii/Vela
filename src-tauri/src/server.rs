//! Local event server: receives vela-hook's POST /event?e=<event>&ppid=<pid>
...
pub fn start(app: AppHandle, port: u16) {
    std::thread::spawn(move || {
        let server = match tiny_http::Server::http(("127.0.0.1", port)) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("[vela] failed to bind port {port}: {e} (is another instance running?)");
                return;
            }
        };
        for mut req in server.incoming_requests() {
            ...
            if url.starts_with("/event") {
                // vela-hook only ever POSTs. A GET is also what a web page can send with no
...
///
/// Native callers like `vela-hook` communicate directly over loopback TCP without
...
mod tests {
    ...
    fn allowed_loopback_origins() {
        assert!(is_allowed_origin("http://127.0.0.1:48667"));
        ...
        assert!(is_allowed_origin("https://127.0.0.1:48667"));
        assert!(is_allowed_origin("https://localhost:3000"));
        assert!(is_allowed_origin("127.0.0.1:48667"));
    ...
    fn hook_request_without_origin_is_allowed() {
        let headers = [
            tiny_http::Header::from_bytes(&b"Host"[..], &b"127.0.0.1:48667"[..]).unwrap(),
    ...
    fn multiple_origins_are_forbidden() {
        let headers = [
            ...
            tiny_http::Header::from_bytes(&b"Origin"[..], &b"http://127.0.0.1:48667"[..]).unwrap(),
        ];
        assert!(is_forbidden_headers(&headers));
    }
