...
fn login_command(cli: &std::path::Path) -> Result<Command, String> {
    ...
    cmd.args(["-NoLogo", "-NoProfile", "-Command", LOGIN_SCRIPT]).env("VELA_CLAUDE_CLI", cli);
...
fn login_command(cli: &std::path::Path) -> Result<Command, String> {
    ...
    let shell = format!("env -u CLAUDECODE -u CLAUDE_CONFIG_DIR -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN {} auth login --claudeai", crate::platform::shell_quote(path));
    // Script source is constant. Shell text travels as an AppleScript argv value.
    let mut cmd = Command::new("/usr/bin/osascript");
    cmd.args(["-e", r#"on run argv
...
fn login_command(_cli: &std::path::Path) -> Result<Command, String> {
...
pub fn start_login() -> Result<(), String> {
    ...
    *message() = "Complete sign-in in the browser or terminal window.".into();
    std::thread::spawn(move || {
        ...
        *message() = if ok { "Login window finished. Refreshing usage to verify sign-in..." } else {
...
mod tests {
    ...
    fn paths_are_data_not_shell_source() {
        ...
        assert!(cmd.get_envs().any(|(k,v)| k == "VELA_CLAUDE_CLI" && v == Some(path.as_os_str())));
    }
    #[test]
    #[cfg(windows)]
