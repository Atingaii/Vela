...
FEATURES=[
 ...
 ('ownership','本地持有与备份','Local ownership & backup','工程数据保留在自己手里。','Keep your engineering data yours.','原生 AppKit、系统 WKWebView 与 Swift helper。Markdown 资产和本地 SQLite 索引保留在本机。CLI 可备份并恢复到新的本地 Store；桌面管理入口仍在完善。完整灾难恢复矩阵仍在验收。提供中英文界面、通知偏好和声音预览；系统通知受 macOS 授权限制。','Native AppKit, system WKWebView and a Swift helper. Keep Markdown assets and SQLite indexes locally. The CLI can back up and restore to a new local Store; desktop management is still being completed. The full disaster-recovery matrix remains under validation. Choose English or Simplified Chinese, notification preferences and sound previews; system delivery requires macOS authorization.')]
CASES=[
