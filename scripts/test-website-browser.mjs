...
const result = { generatedAt: new Date().toISOString(), dist, baseUrl: requestedUrl || null, output, sourceHash: {}, pages: [], screenshots: [], failures: [], warnings: [] };
...
async function scrollAndVerifyImages(page, path) {
  const images = page.locator('img');
  const count = await images.count();
  // Keep native loading="lazy" behavior intact. Each image enters the real viewport
  // before its decode state is inspected; neither src nor loading is mutated.
  for (let index = 0; index < count; index += 1) {
    const image = images.nth(index);
    await image.scrollIntoViewIfNeeded();
    try {
      await page.waitForFunction((imageIndex) => {
        const candidate = document.images[imageIndex];
        return Boolean(candidate && candidate.complete && candidate.naturalWidth > 0);
      }, index, { timeout: 5000 });
    } catch {
      const details = await image.evaluate((candidate) => ({ src: candidate.currentSrc || candidate.src, complete: candidate.complete, naturalWidth: candidate.naturalWidth }));
      fail('lazy-image', `${details.src} did not decode after viewport scroll (complete=${details.complete}, naturalWidth=${details.naturalWidth}).`, path);
    }
  }
  const inspected = await page.evaluate(() => [...document.images].map((image) => ({
    src: image.currentSrc || image.src,
    complete: image.complete,
    naturalWidth: image.naturalWidth,
    productScreenshot: new URL(image.currentSrc || image.src, location.href).pathname.startsWith('/assets/vela-'),
  })));
  for (const image of inspected) if (!image.complete || image.naturalWidth === 0) fail('lazy-image', `${image.src} did not decode after viewport scroll.`, path);
  // Full-page screenshots should retain the page's natural theme but start from the
  // same reading position. This does not alter image loading attributes or sources.
...
try {
  ...
  const page = await context.newPage(), probe = await context.newPage(), consoleProblems = [], missingResponses = new Map(), failedLocalImageRequests = new Map();
  for (const p of [page, probe]) {
    p.on('console', (m) => { if (m.type() === 'error') consoleProblems.push(`${p.url()}: ${m.text()}`); });
    p.on('pageerror', (e) => consoleProblems.push(`${p.url()}: ${e.message}`));
    p.on('response', (r) => { try { if (new URL(r.url()).origin === new URL(result.baseUrl).origin && r.status() >= 400) missingResponses.set(r.url(), r.status()); } catch {} });
    p.on('requestfailed', (request) => { try { if (request.resourceType() === 'image' && new URL(request.url()).origin === new URL(result.baseUrl).origin) failedLocalImageRequests.set(request.url(), request.failure()?.errorText || 'request failed'); } catch {} });
  }
  ...
  if (visualOnly) {
    ...
    for (const locale of ['zh', 'en']) for (const kind of ['home', 'comparisons', 'docs']) for (const width of [375, 1440]) {
      ...
      const images = await scrollAndVerifyImages(page, path);
  ...
  for (const [url, error] of failedLocalImageRequests) fail('image-request-failed', `${url} → ${error}`);
  ...
  for (const file of files) {
    ...
    const row = { path, status: response?.status() ?? null, overflow: {}, language: null, images: [] }; result.pages.push(row); if (row.status !== 200) fail('http-status', `Expected 200, received ${row.status}`, path);
    row.images = await scrollAndVerifyImages(page, path);
  ...
  for (const locale of ['zh', 'en']) for (const kind of ['home', 'comparisons', 'docs']) for (const width of [375, 1440]) {
    ...
    await page.goto(result.baseUrl + path, { waitUntil: 'networkidle' });
    const images = await scrollAndVerifyImages(page, path);
    // Earlier theme-persistence coverage can leave this shared page in dark mode.
    // Persist the actual final screenshot theme instead of assuming a light capture.
    const theme = await page.locator('html').getAttribute('data-theme');
    result.screenshots.push({ path, width, theme, images });
    await page.screenshot({ path: join(output, `${locale}-${kind}-${width}.png`), fullPage: true });
  }
  if (consoleProblems.length) for (const message of consoleProblems) fail('browser-console', message);
  }
