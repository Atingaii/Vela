...
def main():
 ...
 try:
  ...
  def openrun(run,title=None):
   click('.nav-link[data-page="workflows"]');wait('document.querySelector(".nav-link.active")?.dataset.page==="workflows"','workflows page unavailable');click('.tab-btn[data-wftab="runs"]');row='tr.clickable-row[data-id="'+run+'"]';wait('!!document.querySelector('+json.dumps(row)+')','actual run row missing');click(row);wait('window.__drawerVisible("#detail-drawer")','drawer did not open')
  ...
  def stale():
   ...
   b('select','#project-selector',project);wait('document.querySelector("#project-selector")?.value==='+json.dumps(project),'Harbor did not settle');close();seed(run_a,'STALE_RUN_A_RESPONSE');v('window.__drawerArm("hold",'+json.dumps(run_a)+')');openrun(run_a);held('run hold absent');openrun(run_b,'Drawer stale target B');wait('document.querySelector("#run-feedback-card-content")?.innerText.includes("DRAWER_CURRENT_RUN_B_RESPONSE")','current run B response missing');v('window.__drawerRelease()');wait('window.__drawerControl.settled===true','run hold did not settle');d=v('(()=>({title:document.querySelector("#drawer-title")?.textContent||"",card:document.querySelector("#run-feedback-card-content")?.innerText||""}))()');assert 'STALE_RUN_A_RESPONSE'not in d['card']and'Drawer stale target B'in d['title'],'late A response overwrote B';cases.append({'case':'run','stateAfterRelease':d})
