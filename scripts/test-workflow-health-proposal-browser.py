...
def main():
 ...
 try:
  ...
  def open_health():
   click('.nav-link[data-page="workflows"]'); wait('document.querySelector(".nav-link.active")?.dataset.page==="workflows"','workflow page absent'); click('#btn-wf-secondary-menu'); wait('document.querySelector("#btn-wf-secondary-menu")?.parentElement?.open===true','secondary workflow menu did not open'); click('#btn-wf-secondary-menu + .action-menu-items button[data-wftab="health"]'); wait('!!document.querySelector("#workflow-health-findings")','Health finding list selector missing')
