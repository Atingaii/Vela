#!/usr/bin/env python3
"""Diagnostic only: determine whether a delayed real feedback prepare overwrites an edit."""
...
def main():
 ...
 out.mkdir(parents=True);(out/'consumer-test-source.py').write_bytes(Path(__file__).read_bytes());ev={'format':'vela-feedback-prepare-race-diagnostic-v1','status':'failed','synthetic':True,'providerRuns':0,'checks':[]};server=driver=None;failure=None
 ...
 try:
  ...
  ev['fixtureRemoved']=not base.exists();ev['uiAfter']={n:sha(ui/n) for n in FILES};ev['helperAfter']=sha(binary);ev['harnessAfter']={x.name:sha(x) for x in harness};ev['sourceUnchanged']=ev.get('uiBefore')==ev.get('uiFixture')==ev.get('uiAfter')and ev.get('helperBefore')==ev.get('helperFixture')==ev.get('helperAfter')and ev.get('harnessBefore')==ev.get('harnessAfter');save()
