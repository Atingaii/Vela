...
const hash=async()=>Object.fromEntries(await Promise.all(files.map(async n=>[n,createHash('sha256').update(await readFile('Sources/VelaApp/Resources/UI/'+n)).digest('hex')])));
...
try {
 ...
 await page.locator('.memory-card .action-menu > summary').first().click();assert.equal(await page.locator('.btn-mem-edit').first().isVisible(),true);await page.keyboard.press('Escape');
 await page.screenshot({path:output+'/memory.png'});result.checks.push({name:'memory-progressive-actions-and-keyboard',passed:true});
 ...
 await page.setViewportSize({width:1250,height:800});await page.locator('[data-page="settings"]').click();await page.locator('#setting-locale').selectOption('en');await page.locator('[data-page="memory"]').click();assert.equal(await page.locator('#page-container').evaluate(e=>e.scrollTop),0);assert.ok(await page.locator('h1').isVisible());await page.screenshot({path:output+'/memory-en.png'});assert.match(await page.locator('h1').innerText(),/Memory/);result.checks.push({name:'english-memory-layout',passed:true});
} catch(e) {result.errors.push(e.stack||String(e));process.exitCode=1;} finally {await browser.close();result.sourceAfter=await hash();result.sourceUnchanged=JSON.stringify(result.sourceBefore)===JSON.stringify(result.sourceAfter);result.passed=result.checks.length===5&&result.errors.length===0&&result.sourceUnchanged;await writeFile(output+'/results.json',JSON.stringify(result,null,2));console.log(JSON.stringify({passed:result.passed,checks:result.checks.length,errors:result.errors,output}));if(!result.passed)process.exitCode=1;}
