...
ROOT=Path(__file__).resolve().parents[1]; FILES=('app.js','i18n.js','app.css','index.html','app-icon.svg','demo.js'); CHECKS=('prepare-cancel-locales','revision-history-objective','drawer-feedback-projection','run-output-contract','stale-lifecycle')
...
def main():
 ...
 out.mkdir(parents=True);(out/'consumer-test-source.py').write_bytes(Path(__file__).read_bytes()); ev={'format':'vela-run-feedback-renderer-v2','status':'failed','synthetic':True,'completeSuite':False,'providerRuns':0,'modelRuns':0,'workflowExecuted':'fixture Git reads only','nativeClaimed':False,'selectedChecks':sorted(chosen),'checks':[]};server=driver=None;top_error=None
 ...
 try:
  ...
  v("window.__rfBaseCall=window.vela.call;window.vela.call=async(m,x={})=>{const r=await window.__rfBaseCall(m,x);return(m==='runs.get'&&window.__rfRunOverride)?Object.assign({},r,window.__rfRunOverride):r};true")
  def open_form():
   click('.nav-link[data-page="workflows"]');wait('document.querySelector(".nav-link.active")?.dataset.page==="workflows"','workflows page unavailable');click('.tab-btn[data-wftab="runs"]');row='tr.clickable-row[data-id="'+run+'"]';wait('!!document.querySelector('+json.dumps(row)+')','fixture run not rendered');click(row);wait('window.__rfVisible("#btn-open-run-feedback")','missingUI: actual terminal run detail has no #btn-open-run-feedback');before=len(calls('runs.feedback.prepare'));click('#btn-open-run-feedback');wait('window.__rfVisible("#run-feedback-review")','missingUI: click did not open #run-feedback-review');wait('window.__rfVisible("#run-feedback-outcome")&&window.__rfVisible("#run-feedback-reason")&&window.__rfVisible("#btn-save-run-feedback")','missingUI: feedback form controls absent');wait('window.__rfCalls.filter(x=>x.method==="runs.feedback.prepare").length>'+str(before),'prepare absent');got=calls('runs.feedback.prepare')[-1];assert got['params']=={'project':project,'runId':run},'prepare shape/project incorrect'
   prepared=got['result'];feedback=prepared.get('feedback');expected=(feedback or {}).get('outcome')
   if expected:
    wait('document.querySelector("#run-feedback-outcome")?.value==='+json.dumps(expected)+'&&document.querySelector("#run-feedback-reason")?.value==='+json.dumps((feedback or {}).get('reason','')),'prepare result did not finish binding the edit form')
   else:
    wait('document.querySelector("#run-feedback-original-state")?.textContent.trim().length>0','prepare result did not finish binding the form state')
   b('screenshot',str(out/'feedback-form-open.png'));return prepared
  def first():
  ...
  def revisions():
   ...
   for outcome,reason in [('good','Synthetic Git review useful.'),('bad','Synthetic reviewer corrected observation.'),('clear','Synthetic reviewer withdrew observation.')]:
    b('select','#run-feedback-outcome',outcome);assert v('document.querySelector("#run-feedback-outcome")?.value==='+json.dumps(outcome)), 'outcome selection did not settle before save';b('fill','#run-feedback-reason',reason);assert v('document.querySelector("#run-feedback-outcome")?.value==='+json.dumps(outcome)), 'outcome changed before save';click('#btn-save-run-feedback');wait('window.__rfCalls.filter(x=>x.method==="runs.feedback.record"&&!x.error).length>'+str(len(rows)),'record missing');rows.append(calls('runs.feedback.record')[-1]['result']);
   ...
   wait('window.__rfVisible("#btn-run-feedback-history")','missingUI: history button absent');click('#btn-run-feedback-history');wait('window.__rfVisible("#run-feedback-history")','missingUI: history pane absent');wait('window.__rfCalls.some(x=>x.method==="runs.feedback.history.list")','history list absent');listed=calls('runs.feedback.history.list')[-1];assert listed['params']=={'project':project,'runId':run,'limit':20},'history initial request shape incorrect';hist=listed['result'];assert len(hist['items'])==20 and hist.get('cursor'),'history fixture must require a second page';
   item='#run-feedback-history [data-history-id="'+hist['items'][0]['historyId']+'"]';wait('!!document.querySelector('+json.dumps(item)+')','missingUI: history entry lacks an operable data-history-id detail control');click(item);wait('window.__rfCalls.some(x=>x.method==="runs.feedback.history.get")','history get detail call absent');detail=calls('runs.feedback.history.get')[-1];assert detail['params']=={'project':project,'id':hist['items'][0]['historyId']},'history detail did not bind selected immutable id'
   if hist.get('cursor'):wait('window.__rfVisible("#btn-more-run-feedback-history")','missingUI: pagination button absent');click('#btn-more-run-feedback-history');wait('window.__rfCalls.filter(x=>x.method==="runs.feedback.history.list").length>=2','history page two absent');next_page=calls('runs.feedback.history.list')[-1];assert next_page['params']=={'project':project,'runId':run,'limit':20,'cursor':hist['cursor']},'history next page did not use only frozen cursor'
   assert rpc('runs.get',{'id':run})==initialRun,'feedback changed run'
   assert rpc('workflows.health',{'project':project})['successRate']==initialHealth['successRate'],'feedback changed successRate'
   assert rpc('inbox.list',{'project':project})==initialApprovals,'feedback changed approval'
   if v('window.__rfVisible("#btn-close-run-feedback-history")'):
    click('#btn-close-run-feedback-history')
    wait('!window.__rfVisible("#run-feedback-history")','history footer close did not hide the modal')
    footer={'stableId':True,'legacyButtonCount':0,'closed':True}
   else:
    legacy_count=v('document.querySelectorAll("#modal-footer button").length')
    assert legacy_count==1,'history footer fallback is not uniquely operable: '+str(legacy_count)
    click('#modal-footer button')
    time.sleep(.25)
    footer={'stableId':False,'legacyButtonCount':legacy_count,'closed':not v('window.__rfVisible("#run-feedback-history")'),'footerVisible':v('window.__rfVisible("#modal-footer button")')}
    assert footer['closed'],'history footer legacy close click did not hide the modal: '+json.dumps(footer)
    raise AssertionError('missingUI: history footer lacks #btn-close-run-feedback-history; legacy footer did close')
   return {'revisions':[1,2,3],'historyListAndPagination':True,'historyGetRequiredByUI21':True,'historyFooterClose':footer,'objectiveUnchanged':True}
  def drawer_projection():
   prep=rpc('runs.feedback.prepare',{'project':project,'runId':run});current=rpc('runs.feedback.record',{'project':project,'runId':run,'runHash':prep['runHash'],'previousFeedbackHash':(prep.get('feedback')or{}).get('feedbackHash'),'outcome':'good','reason':'DRAWER_CURRENT_FEEDBACK_SENTINEL'})
   click('.nav-link[data-page="workflows"]');click('.tab-btn[data-wftab="runs"]');row='tr.clickable-row[data-id="'+run+'"]';wait('!!document.querySelector('+json.dumps(row)+')','run row missing for feedback projection');click(row);wait('!!document.querySelector("#run-feedback-card-content")','run feedback summary card absent');assert v('document.querySelector("#run-feedback-card-content")?.innerText.includes("DRAWER_CURRENT_FEEDBACK_SENTINEL")'),'reopened run drawer did not load current feedback projection';return {'currentRevision':current['revision'],'reasonVisibleAfterReopen':True}
  def output_contract():
   full_hash='a'*64
   def show(override):
    v('window.__rfRunOverride='+json.dumps(override)+';true')
    click('.nav-link[data-page="workflows"]');click('.tab-btn[data-wftab="runs"]');row='tr.clickable-row[data-id="'+run+'"]';wait('!!document.querySelector('+json.dumps(row)+')','run row missing for output contract');click(row);wait('!!document.querySelector('+json.dumps('#detail-drawer [data-i18n="runs.output.title"]')+')','run output card absent')
   show({'output':None,'outputHash':full_hash,'outputDelivery':None})
   assert not v('!!document.querySelector('+json.dumps('#detail-drawer [data-i18n="runs.output.delivered"],#detail-drawer [data-i18n="runs.output.generated"]')+')'),'hash-only empty output received a delivery/generated label'
   details=v('(()=>{const details=document.querySelector('+json.dumps('#detail-drawer [data-i18n="runs.output.hash"]')+')?.closest("details");return {closed:!!details&&!details.open,full:details?.textContent.includes('+json.dumps(full_hash)+')}})()')
   assert details['closed'] and details['full'],'full output hash is not inside a closed technical details disclosure'
   show({'output':{'result':'synthetic delivered'},'outputHash':full_hash,'outputDelivery':{'state':'delivered','target':'file','path':'output/final.md'}})
   assert v('!!document.querySelector('+json.dumps('#detail-drawer [data-i18n="runs.output.delivered"]')+')&&!document.querySelector('+json.dumps('#detail-drawer [data-i18n="runs.output.generated"]')+')'),'delivered object output label incorrect'
   show({'output':{'result':'synthetic prepared'},'outputHash':full_hash,'outputDelivery':{'state':'prepared','target':'file','path':'output/pending.md'}})
   assert v('!!document.querySelector('+json.dumps('#detail-drawer [data-i18n="runs.output.pending"]')+')&&!document.querySelector('+json.dumps('#detail-drawer [data-i18n="runs.output.generated"]')+')'),'prepared object output label incorrect'
   v('window.__rfRunOverride=null;true')
   return {'hashOnlyNoStatus':True,'fullHashClosedTechnicalDetails':True,'deliveredObjectLabel':True,'preparedObjectLabel':True}
  def stale():
   pre=open_form();b('select','#run-feedback-outcome','good');b('fill','#run-feedback-reason','Synthetic stale UI draft.');rpc('runs.feedback.record',{'project':project,'runId':run,'runHash':pre['runHash'],'previousFeedbackHash':pre['feedback']['feedbackHash']if pre.get('feedback')else None,'outcome':'bad','reason':'Synthetic competing current revision.'});before=len(calls('runs.feedback.record'));click('#btn-save-run-feedback');wait('window.__rfCalls.filter(x=>x.method==="runs.feedback.record").length>'+str(before),'stale save did not reach bridge');wait('window.__rfVisible("#run-feedback-review")','stale error must preserve draft');stale_call=calls('runs.feedback.record')[-1];assert stale_call.get('error'),'stale save did not receive a rejected CAS response';assert len(calls('runs.feedback.record'))==before+1,'UI retried stale write'
   b('fill','#run-feedback-reason','Synthetic pending write.');starts=len(v('window.__rfStarted.filter(x=>x.method==="runs.feedback.record")'));v('window.__rfBeforeHold="runs.feedback.record";true');click('#btn-save-run-feedback');wait('window.__rfHeld===true','pre-call save hold failed');v('(()=>{const x=document.querySelector("#btn-save-run-feedback");if(x&&!x.disabled)x.click();return true})()');assert len(v('window.__rfStarted.filter(x=>x.method==="runs.feedback.record")'))==starts+1,'pending double click started another write';b('press','Escape');b('select','#project-selector',beacon);v('window.__rfRelease();true');wait('window.__rfHeld===false','late save release failed');wait('window.__rfCalls.filter(x=>x.method==="runs.feedback.record").length>'+str(before+1),'late save did not settle');assert not v('window.__rfVisible("#run-feedback-review")'),'late project change reopened stale form';return {'staleCASNoRetry':True,'pendingDoubleClickCoalesced':True,'lateProjectDiscarded':True}
  check('prepare-cancel-locales',first);check('revision-history-objective',revisions);check('drawer-feedback-projection',drawer_projection);check('run-output-contract',output_contract);check('stale-lifecycle',stale)
  ev['uiAfter']={x:sha(ui/x)for x in FILES};ev['helperAfter']=sha(binary);ev['harnessAfter']={x.name:sha(x)for x in harness};ev['harnessUnchanged']=ev['harnessBefore']==ev['harnessAfter'];ev['sourceUnchanged']=ev['uiBefore']==ev['uiFixture']==ev['uiAfter']and ev['helperBefore']==ev['helperFixture']==ev['helperAfter']and ev['harnessUnchanged'];ev['selectedChecksPassed']=len(ev['checks'])==len(chosen)and all(x['passed']for x in ev['checks']);ev['completeSuite']=set(chosen)==set(CHECKS)and ev['selectedChecksPassed']and ev['sourceUnchanged']and not any(x.get('pageErrors')for x in ev['checks']);ev['status']='passed' if ev['completeSuite'] else 'failed';save()
  ...
  save()
 if top_error or ev.get('status')!='passed':raise SystemExit(1)
