...
History 已缓存的 source/epoch/raw 数据不被作为原文删除；规则生效后 `history.sources` 过滤，`history.start`、`history.advance`、`history.page` 与 `history.raw` 重新检查并拒绝命中来源，不能由旧 ID 绕过。已保存 Memory 和用户文件不自动删除；来源不可用会由其原有来源校验处理。
