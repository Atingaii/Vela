...
远端两架构真实升级检查 [35852348869](https://github.com/Atingaii/Velo/actions/runs/35852348869) 全部通过（[ARM](signed-update-macos-arm64-35852348869.json)、[Intel](signed-update-macos-x64-35852348869.json)）。两份报告均完成内部验证基底 .0 到已发布 .1 的暂存、安装、重启，安装后版本/包版本及 WebView/IPC/helper/wake 检查通过；providers=false。Mac 原生控制仍报告锁屏，因此最终发行包的四边、收起/展开、拖动、材质及完整设置交互尚未对照验收；真实账户、多屏和硬件刘海也不在上述安装/升级检查覆盖内。全量迁移任务保持进行中，Windows/Linux 仅规划。

## 第九批：Kiro CLI 登录失败分类

Sol/max 对固定 Swift 和当前源码的有界复核发现：Swift 的 Kiro CLI 非零退出返回 needsAuth，迁移实现此前把它与超时统一显示为网络错误。现已让 Unix 进程助手保留退出状态，Kiro 非零退出映射登录错误；成功仍解析原输出，无法启动、超时或输出读取失败仍为暂态错误。共用既有超时、输出上限和进程组回收；其他调用者的 output() 仍对非零返回 None。

离线自有 fake CLI 覆盖未登录非零退出、成功 stderr 卡片、缺失程序和超时；Kiro 定向 10/10、共享进程助手 2/2 通过。Root 复核接口与差异；两文件格式和 git diff 检查通过。整仓格式检查存在既有其他文件差异，未批量改动。此次没有使用真实 Kiro 凭据，暂存测试目录自动清理，保留 Cargo 缓存。
