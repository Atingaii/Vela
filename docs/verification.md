...
### Plan / AI checkpoint and quota streaming correction

Commit `61bf8d7` captured 353 Core/test methods. Its frozen local portable run passed all 353; the [actual GitHub XCTest run](https://github.com/Atingaii/Vela/actions/runs/34743771288) had exactly one failure: a stdout flood returned `timeout`. All plan and file-Watch methods passed. The separate installed-package job, now including the AI SDK middleware, passed. Renderer and packaging remained skipped after the Core failure.

The plan slice adds 18 methods plus a five-group actual-helper check, pairing persisted Codex/Claude calls with success acknowledgements and retaining unknown/failed states and byte-level provenance. A confirmed task state is not evidence that its engineering work passed. The installed AI SDK package passed 17 HTTP/SSE and real-helper checks, 12 base-SDK compatibility checks, and a separately installed historical-SDK compatibility check. Streaming abort, candidate-only capture, a committed write with a lost response, and scope/privacy rejection are covered; model responses were synthetic.

The quota parser previously searched the entire accumulated unterminated line after every pipe read. It now remembers the newline-free prefix and searches only newly appended bytes, resetting the offset after consuming each line. Byte limits, child cleanup, and the original two-second overflow-test timeout remain unchanged. Ten focused methods passed, including the new split UTF-8 / near-limit notification followed by valid protocol replies. Four local stdout-flood observations per version ranged from 1.222–1.472 seconds before and 0.438–0.769 seconds after; these are diagnostic observations on one host, not a general performance guarantee. The isolated snapshots, raw failure logs, results and resource cleanup receipts are retained independently.
