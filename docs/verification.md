...
5. 核对固定 Swift 的 Phone Link 生产 gate 仍关闭；v3 配对、刷新和撤销只在隔离测试模式验证，不作为已开放产品入口。
6. 安装包内 hook 的位置、签名与可执行权限；Mac 的安装 smoke、更新验签和真实升级分别留证。Gatekeeper/公证独立记录；当前为未公证预览，首次单独允许不能写成默认信任。
