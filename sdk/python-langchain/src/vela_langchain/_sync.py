...
class VelaChatModel:
    def __init__(self, client: Runnable, binding: MemoryBinding):
        ...
        self._closed = False
        self._turns: set[ChatTurn] = set()

    def for_turn(self, *, session_id: str, turn_id: str, timeout: float = 120, cancel_event: threading.Event | None = None) -> ChatTurn:
        with self._lock:
            if self._closed:
            ...
            if cancel_event is not None and not isinstance(cancel_event, threading.Event):
                raise VelaLangChainError("invalid_input")
            turn = ChatTurn(self, bounded_text(session_id, 300), bounded_text(turn_id, 300), duration(timeout), cancel_event)
    ...
    def __enter__(self) -> VelaChatModel:
        return self
...
class ChatTurn:
    def __init__(self, manager: VelaChatModel, session: str, turn: str, timeout: float, event: threading.Event | None):
    ...
    def settled(self, timeout: float | None = None) -> MemoryReceipt:
        ...
        if not current.done.wait(timeout):
            raise VelaLangChainError("timeout")
    ...
    def _release(self, call: _Call) -> None:
        ...
        if response is not None:
            try:
                ...
                # A generator cannot close while another thread is executing it.
                # Keep ownership and retry when the operation unwinds.
                with self._lock:
    ...
    def _prepare(self, call: _Call, value: Any, options: dict[str, Any], streaming: bool) -> dict[str, Any]:
        ...
        call.request_model = snapshot(self._manager._client, self._binding, self._remaining(call))
    ...
    def create(self, *, input: Any, **options: Any) -> AIMessage:
        call = self._begin()
        try:
            prepared = self._prepare(call, input, options, False)
            self._guard(call)
            call.report["model_calls"] += 1
            response = call.request_model.invoke(**prepared)
            self._guard(call)
            ...
            if isinstance(error, VelaLangChainError):
                raise
            raise VelaLangChainError(call.reason if call.stopped.is_set() else "model_failed") from None
        finally:
            call.operation_running = False

    def stream(self, *, input: Any, **options: Any) -> ChatMemoryStream:
        call = self._begin()
        try:
            prepared = self._prepare(call, input, options, True)
            self._guard(call)
            call.report["model_calls"] += 1
            response = call.request_model.stream(**prepared)
            call.response = response
            self._guard(call)
            return ChatMemoryStream(self, call, response)
        except Exception as error:
            self._finish(call, "cancelled" if self._is_cancelled(call) else "failed")
            if isinstance(error, VelaLangChainError):
                raise
...
class ChatMemoryStream(Iterator[AIMessageChunk]):
    def __init__(self, turn: ChatTurn, call: _Call, stream: Any):
        self._turn, self._call, self._stream = turn, call, stream
        self._iterator = iter(stream)

    def __iter__(self) -> ChatMemoryStream:
        return self
    ...
    def __enter__(self) -> ChatMemoryStream:
        return self
