...
Both clients expose `list_projects`, `register_project`, `list_memories`, `recall`, `save_candidate`, `save_candidates`, `prepare_session_capture`, `capture_session_candidate`, `export_archive`, `validate_archive`, and `import_archive`. Memory writes/imports create candidates; activation is a separate human review operation in Vela. Recall uses the selected helper's actual local capabilities, not a SDK-side semantic model.
