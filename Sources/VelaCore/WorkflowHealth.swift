...
extension AutomationService {
    ...
    func workflowHealthReport(_ params: JSON) throws -> JSON {
        ...
        let scannedFeedback = try (root.map { try store.list("run_feedback",project:$0,limit:Self.healthSourceScanCap) } ?? store.list("run_feedback",limit:Self.healthSourceScanCap))
        let feedbackCapReached = scannedFeedback.count == Self.healthSourceScanCap
        let feedbackRows = try verifiedManualFeedback(rows:scannedFeedback,runs:nonDry)
        let feedbackByRun = Dictionary(grouping:feedbackRows,by:{ string($0,"runId") })
        let runIDs = Set(nonDry.map { string($0,"id") })
        let approvals = scannedApprovals.filter { runIDs.contains(string($0,"runId")) }
        let aggregateIncomplete = runCapReached || approvalCapReached || feedbackCapReached
        let states = ["completed","failed","cancelled","rejected","expired","pending_approval","running","needs_review"]
        var result: JSON = ["workflowId":id,"runs":nonDry.count,"completedRuns":terminal.count,"successes":successes.count,"failures":terminal.filter { string($0,"state") == "failed" }.count,"successRate":terminal.isEmpty ? NSNull() : Double(successes.count)/Double(terminal.count),"averageDurationMs":durations.isEmpty ? NSNull() : durations.reduce(0,+)/Double(durations.count),"durationAvailable":!durations.isEmpty,"durationSamples":durations.count,"durationMissing":terminal.count-durations.count,"approvalRejected":approvals.filter { string($0,"state") == "rejected" }.count,"approvalExpired":approvals.filter { string($0,"state") == "expired" }.count,"expiredRuns":nonDry.filter { string($0,"state") == "expired" }.count,"tokens":NSNull(),"tokensAvailable":false,"guidelineInfluence":"not_measured","manualFeedback":["observed":feedbackRows.filter { ["good","bad"].contains(string($0,"outcome")) }.count,"good":feedbackRows.filter { string($0,"outcome") == "good" }.count,"bad":feedbackRows.filter { string($0,"outcome") == "bad" }.count,"clear":feedbackRows.filter { string($0,"outcome") == "clear" }.count,"meaning":"manual_observation_not_objective_outcome"],"window":["since":params["since"] ?? NSNull(),"workflowVersion":version as Any? ?? NSNull(),"dryRunsExcluded":all.count-nonDry.count],"sourceScan":["cap":Self.healthSourceScanCap,"scope":root == nil ? "all_projects" : "project","runs":["scanned":scannedRuns.count,"capReached":runCapReached],"approvals":["scanned":scannedApprovals.count,"capReached":approvalCapReached],"manualFeedback":["scanned":scannedFeedback.count,"capReached":feedbackCapReached]],"aggregateIncomplete":aggregateIncomplete,"coverage":aggregateIncomplete ? "bounded_by_source_scan_cap" : "complete_within_selected_store_scope","versionGroups":healthGroups(nonDry,states:states),"stateCounts":healthStateCounts(nonDry,states:states),"detailAvailability":root == nil ? "requires_project" : "project_scoped","items":[] as [JSON],"findings":[] as [JSON],"cursor":NSNull(),"truncated":aggregateIncomplete]
        ...
        result["items"] = page.map { healthSummary($0,manualFeedback:feedbackByRun[string($0,"id")] ?? []) }; result["findings"] = page.flatMap(healthFindings); result["cursor"] = pageMore ? string(page.last ?? [:],"id") : NSNull(); result["truncated"] = pageMore || aggregateIncomplete
        return result
    }
    ...
    private func healthSummary(_ run: JSON, manualFeedback: [JSON] = []) -> JSON {
    ...
    private func healthFindings(_ run: JSON) -> [JSON] {
        ...
        let concluded = steps.filter { ["completed","failed","rejected","expired","cancelled","needs_review"].contains(string($0,"state")) }
