...
final class MemoryIntegrationService {
    ...
    func handle(_ method: String, _ params: JSON) throws -> JSON {
        ...
        case "memory.integration.capture":
            ...
            guard string(params,"integration") == "openclaw", JSONSerialization.isValidJSONObject(params), try jsonString(params).utf8.count <= 64 * 1024,
            ...
            for record in records {
                ...
                let identity: JSON = ["project":project,"namespace":namespace,"integration":"openclaw","sourceID":sourceID,"sourceMessage":sourceMessage,"contentSHA256":stableHash(content),"role":string(record,"role")]
        ...
        case "memory.integration.stats":
            ...
            return ["project":project,"namespace":namespace,"observedRecords":rows.count,"states":Dictionary(grouping:rows,by:{string($0,"state")}).mapValues(\.count),"complete":page.count < 10000,"limit":10000,"namespaceIsRemoteACL":false]
