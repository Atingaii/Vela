...
```sh
python3 scripts/version-website-assets.py
python3 scripts/check-repository.py
cd website
npx --yes wrangler@4.131.1 pages deploy dist --project-name velo --branch main
```

The custom domain must be associated with the Pages project, not just pointed at its `pages.dev` hostname. Do not change nameservers or unrelated DNS records. Deployment is explicit; a GitHub push alone does not publish the site. HTML requests revalidate with `Cache-Control: public, max-age=0, must-revalidate`. Cloudflare can impose a longer browser TTL on static assets, so `version-website-assets.py` stamps local CSS, JavaScript and images with content-derived version queries. Run it after any asset edit and before deployment; the catalogue generator also invokes it. No duplicate asset files or runtime loader is added. The deployment verifier compares page and resource bytes with the local source snapshot.
