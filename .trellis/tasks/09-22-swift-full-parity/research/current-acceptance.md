...
已将 `v0.1.1-preview.1` 标签固定到 `6859771`，触发 [发行检查 35842708336](https://github.com/Atingaii/Velo/actions/runs/35842708336)。该次 browser 与 Apple Silicon DMG 安装检查通过，但 Intel 在 Rust tests、release 主程序构建和 .app 签名成功后，bundle_dmg.sh 约 22 秒失败，尚未执行 Intel 安装 smoke。publish 已跳过，Release/feed 未发布。Root 已验证同一 ARM DMG 在本机独立安装后成功启动；报告已入库。后续使用固定源码验证入口补充 verbose 打包日志，并启用真正的 cache-on-failure；不移动现有标签、不放宽安装门槛。官网 `6859771` 平台文案已部署至 Cloudflare（部署 `7d242192`），首页、下载、产品、指南和样式均与本地字节一致；下载仍指向确实存在的旧 `.4`，新链接待资产成功发布后更新。
