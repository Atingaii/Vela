//! vela-hook: the minimal client Claude Code's hooks call.
...
const DEFAULT_PORT: u16 = 48667;
const MAX_STDIN: u64 = 256 * 1024;

fn main() {
    // Bound stdin, socket retries and launch together, including a stalled hook caller.
    std::thread::spawn(|| {
...
/// Read the shared platform config directory and validate the configured port.
fn read_port() -> u16 {
    dirs::config_dir()
        .and_then(|d| std::fs::read(d.join("vela/config.json")).ok())
...
fn send(port: u16, event: &str, ppid: u32, body: &str) -> std::io::Result<()> {
    ...
    if buf[..n].starts_with(b"HTTP/1.1 200") || buf[..n].starts_with(b"HTTP/1.0 200") {
        ...
        Err(std::io::Error::other("hook delivery was not acknowledged"))
    }
}
